  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-4 bg-background">
      ...
      >
        <Card className="shadow-2xl rounded-xl border-border/20 bg-card/80 backdrop-blur-sm">
          <CardHeader className="text-center p-8">
            ...
            <motion.div variants={FADE_UP_ANIMATION_VARIANTS}>
              <CardTitle className="text-4xl font-bold tracking-tight font-headline text-primary">DocuMind</CardTitle>
              <CardDescription className="text-lg mt-2 text-muted-foreground">Your intelligent document assistant.</CardDescription>
            </motion.div>
          </CardHeader>
          <CardContent className="p-8 pt-0">
            <motion.div variants={FADE_UP_ANIMATION_VARIANTS}>
              <Button onClick={handleLogin} disabled={isLoggingIn} className="w-full h-12 text-lg" variant="shine">
                {isLoggingIn ? (
        ...
        <motion.footer variants={FADE_UP_ANIMATION_VARIANTS} className="mt-8 text-center text-sm text-muted-foreground">
